...
도구나 AI agent가 이해하기 쉬운 실행 절차입니다.
...
기계, 자동화 도구, 이후의 AI 검토가 읽기 쉬운 결과 기록입니다.
...
6. `templates/result-template.json`을 사용해 예시 결과를 기록합니다.
7. `examples/`의 완성 예시와 비교합니다.
8. `references/validation-checklist.md`로 제출 준비 상태를 확인합니다.
9. workflow의 한 부분을 개선하고 반복합니다.

## 예시를 수동으로 실행하는 방법

이 저장소는 코드를 실행하지 않아도 검토할 수 있습니다. 예시 하나를 수동으로 확인하려면 다음 순서로 보면 됩니다.

1. `examples/meeting-to-actions` 같은 예시 폴더를 엽니다.
...
`intent2ops-skill`은 비개발자가 반복되는 지식 업무를 재사용 가능한 AI-agent workflow로 바꾸도록 돕는 Skillathon 제출물입니다. Markdown으로 사람의 의도, YAML로 실행 절차, JSON으로 machine/AI-friendly 결과를 분리합니다. templates, mock data, 세 가지 완성 예시, 검증 체크리스트를 포함해 실제 자동화 도구를 연결하기 전에 안전하게 연습할 수 있습니다.
